...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!--  Output the bionet schema as skos rdf. Revision of bionet17. More concise  -->
   ...
   <xsl:template match="/">
    ...
    <xsl:template match="doc">
        <rdf:RDF>
            ...
            <!--  Describe narrower concepts of the top concepts       -->
            <xsl:apply-templates select="section/term"/>
            <!--  Describe narrower concepts of the narrower concepts of the top concepts  -->
            <xsl:apply-templates select="section/term/term"/>
    ...
    <!-- Describe narrower concepts of the top concepts   -->
    <xsl:template match="section/term | section/term/term">
        <rdf:Description rdf:about="{concat($schemaBaseURI,'/',@id)}">
            <rdf:type rdf:resource="{$skosConceptURI}"/>
            <skos:prefLabel xml:lang="{$language}"><xsl:value-of select="@name"/></skos:prefLabel>
            <dcterms:creator><xsl:value-of select="$creator"/></dcterms:creator>
            ...
            <xsl:apply-templates select="intro"/>
            <xsl:apply-templates select="term" mode="narrower"></xsl:apply-templates>
            <xsl:if test="exists(@childVocabName)">
